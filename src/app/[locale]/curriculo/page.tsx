...
export default async function CurriculoPage() {
  ...
  return (
    <>
      {/* Print CSS: hide shell, enforce A4 */}
      <style>{`
        @media print {
          header,
          footer,
      ...
      <div className="cv-root" style={{ color: C.navy, fontFamily: 'var(--font-sans, system-ui)' }}>
        ...
        <div style={{ borderBottom: `2px solid ${C.navy}`, paddingBottom: '1.25rem', marginBottom: '1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
            <div>
              <h1 style={{ fontSize: '26px', fontWeight: 700, lineHeight: 1, letterSpacing: '-0.5px', margin: 0, color: C.navy }}>
                Marcelle Rocha
              </h1>
              <p style={{ fontSize: '11px', fontWeight: 400, letterSpacing: '0.8px', textTransform: 'uppercase', color: C.subtle, marginTop: '5px' }}>
                Product Designer · UX Strategy
              </p>
            </div>
            <div style={{ textAlign: 'right', fontSize: '11px', color: C.subtle, lineHeight: 1.6 }}>
              <p style={{ margin: 0 }}>marcelle.rdoss@gmail.com</p>
              <p style={{ margin: 0 }}>linkedin.com/in/marcelle-rocha-dos-santos</p>
              <p style={{ margin: 0 }}>Rio de Janeiro, RJ</p>
            </div>
          </div>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div className="avoid-break">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '6px' }}>
                {education.map(item => (
                  <div key={item.name}>
                    <p style={{ margin: 0, fontSize: '10px', fontWeight: 500, color: C.navy, lineHeight: 1.4 }}>{item.name}</p>
              ...
              {experience.map((item, i) => (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: '8px', marginBottom: '2px' }}>
                    <p style={{ margin: 0, fontSize: '12px', fontWeight: 560, color: C.navy, lineHeight: 1.35, letterSpacing: '-0.1px' }}>
                      {item.role.pt}
                    </p>
                    <p style={{ margin: 0, fontSize: '10px', color: C.magenta, whiteSpace: 'nowrap', flexShrink: 0, fontWeight: 400 }}>
                      {item.period}
                    </p>
                  </div>
                  <p style={{ margin: 0, fontSize: '10.5px', color: C.subtle, marginBottom: '5px', fontWeight: 400 }}>
                    {item.company}
                  </p>
        ...
        {/* ── Footer strip ──────────────────────────────────── */}
        <div style={{ marginTop: '2rem', paddingTop: '0.75rem', borderTop: `1px solid ${C.border}` }}>
          <p style={{ margin: 0, fontSize: '9px', color: '#aaaaaa', letterSpacing: '0.4px', textTransform: 'uppercase', textAlign: 'center' }}>
            marcellerocha.com.br · Portfolio completo disponível online
          </p>
        </div>

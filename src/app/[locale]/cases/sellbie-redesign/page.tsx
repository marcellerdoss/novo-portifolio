...
function Panel({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`w-screen h-screen flex-shrink-0 overflow-hidden flex items-center gap-16 px-20 pt-20 pb-10 ${className}`}>
      {children}
    </div>
  );
}
...
function TextCol({ children }: { children: React.ReactNode }) {
  return <div className="w-[360px] shrink-0 flex flex-col gap-4">{children}</div>;
}

function VisualCol({ children }: { children: React.ReactNode }) {
  return (
...
export default async function SellbieRedesignPage({ params }: Props) {
  ...
  return (
    <HorizontalScroll backLabel={en ? 'Back' : 'Voltar'}>
      ...
      {/* 08 — Resultado */}
      <Panel className="flex-col items-center justify-center gap-12">

...
function Panel({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`w-screen h-screen flex-shrink-0 overflow-hidden flex ${className}`}>
      {children}
    </div>
  );
}
...
// Text-only panel — content left-aligned, vertically centered, readable max-width
function TextPanel({ children }: { children: React.ReactNode }) {
  return (
    <Panel>
      <div className="flex flex-col justify-center gap-5 pl-20 pr-16 pt-20 pb-10 max-w-[520px]">
        {children}
      </div>
    </Panel>
  );
...
// Image panel — fills the panel, image centered and constrained
...
function ImagePanel({ src, alt, caption }: { src: string; alt: string; caption?: string }) {
  return (
    <Panel className="justify-center">
      <div className="w-full max-w-[1048px] flex flex-col pt-14 pb-6 pr-10 pl-10 overflow-hidden">
...
function SliderPanel({ before, after, caption }: {
  ...
  return (
    <Panel className="justify-center">
      <div className="w-full max-w-[1048px] flex flex-col pt-14 pb-6 pr-10 pl-10 overflow-hidden">
        <BeforeAfterSlider before={before} after={after} caption={caption} />
      </div>
    </Panel>
  );
}
...
export default async function SellbieRedesignPage({ params }: Props) {
  ...
  return (
    <HorizontalScroll backLabel={en ? 'Back' : 'Voltar'}>
      ...
      <ImagePanel src="/images/cases/sellbie/redesign/sellbie-redesign-navegacao-antes-1.png" alt={en ? 'Top navigation pattern' : 'Padrão de navegação superior'} />
      ...
      <ImagePanel src="/images/cases/sellbie/redesign/sellbie-redesign-board-entrevistas.png" alt={en ? 'Interview clustering board' : 'Board de clusterização das entrevistas'} />
      ...
      <ImagePanel src="/images/cases/sellbie/redesign/sellbie-redesign-envio-drawer-email-etapa-resumo.png" alt={en ? 'Consolidated summary' : 'Resumo consolidado'} />
